...
## Brand files (compose jobs)

If the job names a brand (e.g. RETARD GLOBAL), load the compact JSON via image-compose `scripts/brand.py` before drawing. Search: `--brand` / `BRAND_FILE` → `<cwd>/brands/` → `~/Pictures/cli-anything-poster/brands/` → skill `brands/`. Use those hexes for straps, bands, CTA plates. Native `gimp -i -b` hangs on this box — stay on the Pillow render path when draw ops exist.
